...
Assembler.prototype.loadReporters = function (callback) {
  ...
  var queue = async.queue(function (reporterSettings, callback) {
    ...
    var eventBusReceiverOptions = {
      queueName: reporterSettings.eventBusReceiver.queue,
      logger: self.logger,
      scope: self.configuration.scope
    };

    if (reporterSettings.options) {
      var options = reporterSettings.options;
      for (var key in options) {
        eventBusReceiverOptions[key] = options[key];
      }
    }

    var eventBusReceiver = new EventBusReceiver(eventBusReceiverOptions);
    var reporterOptions = {
