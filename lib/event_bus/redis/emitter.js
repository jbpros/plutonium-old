...
RedisEventBusEmitter.prototype.clearQueues = function (callback) {
  var self = this;
  var queueKeyMatcher = self.queue.getQueueKeyPrefix() + "*";
...
RedisEventBusEmitter.prototype._start = function (callback) {
  ...
  async.series([
    function (next) {
      self.queueManager = redis.createClient(self.redisPort, self.redisHost);
    ...
    function (next) {
      self.queueWriter = redis.createClient(self.redisPort, self.redisHost);
