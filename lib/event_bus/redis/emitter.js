...
RedisEventBusEmitter.prototype.emit = function (event, callback) {
  ...
  self._start(function (err) {
    ...
    self.queueManager.smembers(self.queue.queueSetKey(), function (err, queueNames) {
      if (err) return callback(err);
      ...
      queueNames.forEach(function (queueName) {
        var key = self.queue.inQueueListKeyPrefix() + queueName;
...
RedisEventBusEmitter.prototype.clearQueues = function (callback) {
  var self = this;
  var QUEUE_KEY_MATCHER = self.queue.queueKeyPrefix() + "*";
