...
var QUEUE_KEY_SUBSCRIBED       = "events" + QUEUE_KEY_SEPARATOR;
var QUEUE_KEY_REPLAYING_PREFIX = "replay" + QUEUE_KEY_SEPARATOR;
...
RedisEventBusQueue.prototype.getQueueKeyPrefix = function () {
  var result = QUEUE_KEY_PREFIX;

  if (this.scope)
    result = this.scope + QUEUE_KEY_SEPARATOR + result;

  if (this.replaying)
    result += QUEUE_KEY_REPLAYING_PREFIX;
...
RedisEventBusQueue.prototype.getSubscribedQueueKeyPrefix = function () {
  return this.getQueueKeyPrefix() + QUEUE_KEY_SUBSCRIBED;
};

RedisEventBusQueue.prototype.getSubscribedQueueKey = function () {
  return this.getQueueKeyPrefix() + QUEUE_KEY_SUBSCRIBED + this.name;
};
...
RedisEventBusQueue.prototype.start = function (callback) {
  ...
  if (!self.canStart)
    return callback(new Error("Queue cannot start twice, use another instance"));
  else
    self.canStart = false;

  function readEvent() {
    if (self.stopped) {
      self.logger.debug("RedisEventBusQueue", "STOPPED - skipping event reading ");
      return;
    }
    ...
    self.queueReader.lrange(outKey, LAST_KEY_INDEX, LAST_KEY_INDEX, function (err, results) {
      if (self.stopped) {
        self.logger.debug("RedisEventBusQueue", "STOPPED - skipping event processing ");
        return;
      }
      ...
      if (serializedEvent) {
        ...
        self.logger.log("RedisEventBusQueue", "got event \"" + event.name + "\" (" + event.uid + ") from entity " + event.entityUid + " (queue: " + self.name + ")");
  ...
  self.logger.debug("RedisEventBusQueue", "registering queue \""  + self.name + "\"");
...
RedisEventBusQueue.prototype.subscribeEvents = function (eventNames, callback) {
  var self        = this;
  self.queueManager.hkeys(self.getSubscribedQueueKey(), function(err, oldEventNames){
    if (err) return callback(err);
    var transaction = self.queueManager.multi();
    eventNames.forEach(function(eventName){
      transaction.hset(self.getSubscribedQueueKey(), eventName, true);
    });
    oldEventNames.forEach(function(oldEventName){
      oldEventName = oldEventName.toString();
      if (eventNames.indexOf(oldEventName) == -1) {
        transaction.hdel(self.getSubscribedQueueKey(), oldEventName);
      }
    });
    transaction.exec(function (err, _) {
      callback(err);
