...
RedisEventBusQueue.prototype.initialize = function (callback) {
  ...
  async.series([
    function (next) {
      self.queueManager = redis.createClient(self.redisPort, self.redisHost, { return_buffers: true });;
    ...
    function (next) {
      self.queueReader = redis.createClient(self.redisPort, self.redisHost, { return_buffers: true });;
