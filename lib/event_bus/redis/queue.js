...
RedisEventBusQueue.prototype.start = function (callback) {
  ...
  function readEvent() {
    self.logger.debug("RedisEventBusQueue", "reading event from queue " + inKey);
    self.queueReader.lrange(outKey, LAST_KEY_INDEX, LAST_KEY_INDEX, function (err, results) {
      ...
      if (serializedEvent) {
        ...
        self.handler(event, function (err) {
          if (err) {
            self.logger.warning("RedisEventBusQueue", "(reading event) an error occurred (" + err + "), " + self.retries + " retries.");
            self.retries++;
          } else {
